...
{     
      ...
      Eigen::MatrixXd vertex; // 8*3, 8个顶点,3轴坐标
...
*/                                                                                 
      ...
      // 将cube向四周膨胀0.5*resolution_, 保证每个顶点在grid中心
      ...
      void setBox()
      {
            box.clear();
            box.resize(3);
            // box存储的是cube在x, y, z轴的长度
            box[0] = std::make_pair( vertex(3, 0), vertex(0, 0) );  // p4 p1
            box[1] = std::make_pair( vertex(0, 1), vertex(1, 1) );  // p1 p2
            box[2] = std::make_pair( vertex(4, 2), vertex(0, 2) );  // p5 p1
      }
...
{     
   ...
   double occupancy;    // 1 --> occupied, 0 --> free
